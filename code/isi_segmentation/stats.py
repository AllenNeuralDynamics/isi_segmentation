...
class ISIMetricsAndVisualization:
    ...
    def __init__(
        ...
        self.retinotopy_altitude_height = self.data.retinotopy_altitude_shape[
            0
        ]
    ...
    def generate_qc_metric_and_images(
        ...
        logging.info(
            f"Found {len(structure_ids)} unique structure IDs in label map."
        )
        ...
        data = window_level(
            vasculature, np.min(vasculature), np.max(vasculature)
        )
        ...
        # resize mask
        outline_im_ten = Image.fromarray(
            np.uint8(target_mask_ten * line_alpha)
        )
        outline_im_ten = outline_im_ten.resize(background_im.size)

        outline_im_five = Image.fromarray(
            np.uint8(target_mask_five * line_alpha)
        )
        outline_im_five = outline_im_five.resize(background_im.size)

        outline_im_one = Image.fromarray(
            np.uint8(target_mask_one * line_alpha)
        )
