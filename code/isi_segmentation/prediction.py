...
def predict(
    ...
    sign_map = normalize_sign_map_forpred(data.visual_sign)
    cv2.imwrite(sign_map_path, sign_map)
    logging.info(f"Saving normalized sign map to {sign_map_path}")

    if not os.path.isfile(sign_map_path):
        raise FileNotFoundError("sign_map_path not a valid file")

    logging.info(f"Load the sign map from {sign_map_path}")
    logging.info("---" * 20)

    # Get the input sign map shape
    sign_map = cv2.imread(
        sign_map_path, cv2.IMREAD_GRAYSCALE
    )  # sign image shape: (540, 640)

    # ----------------------------------
    # Read in the sign map for prediction
    # ----------------------------------

    image = read_img_forpred(
        sign_map_path
    )  # resize sign map to shape (512, 512) for prediction
    ...
    logging.info("Prediction duration: {}".format(end_time - start_time))
    ...
    pred = cv2.resize(
        pred,
        (sign_map.shape[1], sign_map.shape[0]),
    ...
    logging.info("Run post-processing ...")
    closeIter = 5
    openIter = 5
